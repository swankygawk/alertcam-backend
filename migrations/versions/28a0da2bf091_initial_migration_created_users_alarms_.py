"""Initial migration. Created users, alarms and alarm_events tables.

Revision ID: 28a0da2bf091
Revises: 
Create Date: 2025-05-08 02:49:06.372207

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '28a0da2bf091'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('users',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('username', sa.String(length=64), nullable=False),
    sa.Column('password_hash', sa.String(length=256), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('users', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_users_username'), ['username'], unique=True)

    op.create_table('alarms',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('user_id', sa.Integer(), nullable=False),
    sa.Column('vehicle_track_id', sa.Integer(), nullable=False),
    sa.Column('initial_bbox_x1', sa.Float(), nullable=True),
    sa.Column('initial_bbox_y1', sa.Float(), nullable=True),
    sa.Column('initial_bbox_x2', sa.Float(), nullable=True),
    sa.Column('initial_bbox_y2', sa.Float(), nullable=True),
    sa.Column('set_at', sa.DateTime(), nullable=True),
    sa.Column('unset_at', sa.DateTime(), nullable=True),
    sa.Column('is_active', sa.Boolean(), nullable=True),
    sa.Column('last_notification_at', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('alarms', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_alarms_is_active'), ['is_active'], unique=False)
        batch_op.create_index(batch_op.f('ix_alarms_set_at'), ['set_at'], unique=False)
        batch_op.create_index(batch_op.f('ix_alarms_user_id'), ['user_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_alarms_vehicle_track_id'), ['vehicle_track_id'], unique=False)

    op.create_table('alarm_events',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('alarm_id', sa.Integer(), nullable=False),
    sa.Column('event_type', sa.String(length=64), nullable=False),
    sa.Column('timestamp', sa.DateTime(), nullable=True),
    sa.Column('details_json', sa.Text(), nullable=True),
    sa.ForeignKeyConstraint(['alarm_id'], ['alarms.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('alarm_events', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_alarm_events_alarm_id'), ['alarm_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_alarm_events_event_type'), ['event_type'], unique=False)
        batch_op.create_index(batch_op.f('ix_alarm_events_timestamp'), ['timestamp'], unique=False)

    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('alarm_events', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_alarm_events_timestamp'))
        batch_op.drop_index(batch_op.f('ix_alarm_events_event_type'))
        batch_op.drop_index(batch_op.f('ix_alarm_events_alarm_id'))

    op.drop_table('alarm_events')
    with op.batch_alter_table('alarms', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_alarms_vehicle_track_id'))
        batch_op.drop_index(batch_op.f('ix_alarms_user_id'))
        batch_op.drop_index(batch_op.f('ix_alarms_set_at'))
        batch_op.drop_index(batch_op.f('ix_alarms_is_active'))

    op.drop_table('alarms')
    with op.batch_alter_table('users', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_users_username'))

    op.drop_table('users')
    # ### end Alembic commands ###
